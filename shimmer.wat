(module
    ...
    (func $cursor_return 
        (call $print (i32.const 27))
        (call $print (i32.const 91))
        (call $print (call $as_ascii (i32.const 1)))
        (call $print (i32.const 0x3B))
        (call $print (call $as_ascii (i32.const 1)))
        (call $print (i32.const 0x48))
    )
    (func $cursor_to (param $r i32) (param $c i32) 
        (call $print (i32.const 27))
        (call $print (i32.const 91))
        (call $print (call $as_ascii (i32.div_u (local.get $r) (i32.const 10) )))
        (call $print (call $as_ascii (i32.rem_u (local.get $r) (i32.const 10) )))
        (call $print (i32.const 0x3B))
        (call $print (call $as_ascii (i32.div_u (local.get $c) (i32.const 10) )))
        (call $print (call $as_ascii (i32.rem_u (local.get $c) (i32.const 10) )))
        (call $print (i32.const 0x48))
    )
    (func $main (export "_start")
        (local $i i32 )
        (local.set $i (i32.const 800))
        (call $clear_screen)
        (loop $loop
            ;;(call $cursor_return)
            (call $cursor_to (i32.const 3) (i32.const 13))
            (call $print_random_block_matrix_wide (i32.const 25))
            (local.set $i (i32.sub (local.get $i) (i32.const 1))) ;; i = i - 1;
            (br_if $loop (i32.le_s (i32.const 0) (local.get $i) ));; if ($i <= 0) break;
        )
    )
